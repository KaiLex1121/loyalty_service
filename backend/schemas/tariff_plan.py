...
class TariffPlanBase(BaseModel):
    ...
    is_public: bool = True
    is_trial: bool = False
    trial_duration_days: Optional[int] = Field(None, ge=1) # Должно быть > 0, если is_trial=True
    sort_order: int = 0
...
class TariffPlanCreate(TariffPlanBase):
    ...
    def check_trial_duration_on_create(cls, v, values):
        if values.get('is_trial') and (v is None or v < 1):
            raise ValueError('Trial duration in days must be provided and positive for a trial plan.')
