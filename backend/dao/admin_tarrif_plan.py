...
from backend.schemas.tariff_plan import TariffPlanCreate, TariffPlanUpdate # Используем наши схемы
from backend.enums import TariffStatusEnum # Прямой импорт, если не через __init__
...
class TariffPlanDAO(CRUDBase[TariffPlan, TariffPlanCreate, TariffPlanUpdate]):
    async def get_by_name(self, db: AsyncSession, *, name: str) -> Optional[TariffPlan]:
        ...
        result = await db.execute(select(self.model).filter(self.model.name == name, self.model.deleted_at.is_(None)))
    ...
    async def get_trial_plan(self, db: AsyncSession) -> Optional[TariffPlan]:
        ...
        stmt = (
            ...
            .order_by(self.model.created_at.desc()) # Берем самый новый, если их несколько (хотя не должно быть)
        )
        result = await db.execute(stmt)
        return result.scalars().first()

    async def get_all_active_public_plans(self, db: AsyncSession, skip: int = 0, limit: int = 100) -> List[TariffPlan]:
