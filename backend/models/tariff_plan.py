...
class TariffPlan(Base):
    ...
    currency: Mapped[CurrencyEnum] = mapped_column(
        SQLAlchemyEnum(CurrencyEnum, name="currency_enum", create_constraint=False, inherit_schema=True),
        nullable=False,
        default=CurrencyEnum.RUB
    )
    billing_period: Mapped[PaymentCycleEnum] = mapped_column(
        SQLAlchemyEnum(PaymentCycleEnum, name="tariff_payment_cycle_enum", create_constraint=False, inherit_schema=True),
        nullable=False,
        default=PaymentCycleEnum.MONTHLY
    )
    max_outlets: Mapped[Optional[int]] = mapped_column(Integer, default=0, nullable=True)
    max_employees: Mapped[Optional[int]] = mapped_column(Integer, default=0, nullable=True)
    max_active_promotions: Mapped[Optional[int]] = mapped_column(Integer, default=0, nullable=True)
    features: Mapped[Optional[List[str]]] = mapped_column(JSON, nullable=True)
    status: Mapped[TariffStatusEnum] = mapped_column(
        SQLAlchemyEnum(TariffStatusEnum, name="tariff_status_enum", create_constraint=True, inherit_schema=True),
