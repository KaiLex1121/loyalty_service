...
async def add_employee_to_company_endpoint(
    ...
    company: CompanyModel = Depends(get_owned_company), # Зависимость проверяет доступ к company_id и возвращает объект Company
    session: AsyncSession = Depends(get_session), # Сессия нужна сервису для транзакции
    employee_service: EmployeeService = Depends(get_employee_service) # Инжектим сервис
):
    return await employee_service.add_employee_to_company(session, company, employee_data)
...
async def get_company_employees_endpoint(
    ...
    """
    return await employee_service.get_employees_for_company(session, company.id, skip, limit)
...
async def get_employee_by_id_endpoint(
    ...
    session: AsyncSession = Depends(get_session), # Может понадобиться сервису для _build_employee_response
    employee_service: EmployeeService = Depends(get_employee_service)
):
    """
...
async def update_employee_endpoint(
    ...
    employee_role_to_update: EmployeeRole = Depends(get_owned_employee_role), # Зависимость
    session: AsyncSession = Depends(get_session),
    employee_service: EmployeeService = Depends(get_employee_service)
):
    """
    ...
    return await employee_service.update_employee_in_company(session, employee_role_to_update, update_data)
...
async def archive_employee_endpoint(
    employee_role_to_remove: EmployeeRole = Depends(get_owned_employee_role), # Зависимость
    session: AsyncSession = Depends(get_session),
    employee_service: EmployeeService = Depends(get_employee_service)
):
    """
    ...
    return await employee_service.remove_employee_from_company(session, employee_role_to_remove)
