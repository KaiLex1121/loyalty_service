...
from backend.core.dependencies import get_cashback_service, get_dao, get_session, get_owned_company
...
async def get_company_cashback_config_endpoint(
    ...
    company: CompanyModel = Depends(get_owned_company), # Зависимость проверяет права и загружает компанию с cashback_config
    session: AsyncSession = Depends(get_session), # Нужна сервису (хотя в get_cashback_config он ее не использует для запросов)
    ...
    """
    return await cashback_service.get_cashback_config(session, company) # Передаем session, хотя get_cashback_config может ее не использовать
