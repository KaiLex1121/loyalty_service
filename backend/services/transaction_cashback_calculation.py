...
from backend.enums import TransactionTypeEnum, TransactionStatusEnum
from backend.enums import PromotionTypeEnum, CashbackTypeEnum
...
class CashbackCalculationService:
    ...
    async def _find_applicable_promotion(
        ...
        for promotion in active_promotions:
            ...
            if await self._can_apply_promotion(session, promotion, customer_role_id, purchase_amount):
                # Рассчитываем потенциальный кэшбэк для сравнения
                potential_cashback = self._calculate_promotion_cashback_amount(purchase_amount, promotion.cashback_config)
        ...
        applicable_promotions.sort(
            key=lambda x: (x['priority'], x['potential_cashback']),
            reverse=True
        )
    ...
    async def _can_apply_promotion(
        ...
        if (promotion.min_purchase_amount and
            purchase_amount < promotion.min_purchase_amount):
            return False

        # Проверка общего лимита использований
        if (promotion.max_total_uses is not None and
            promotion.current_total_uses >= promotion.max_total_uses):
            return False

        # Проверка лимита использований на клиента
        if promotion.max_uses_per_customer is not None:
            customer_uses = await self.dao.promotion_usage.count_usages_by_customer_for_promotion(
                session,
    ...
    def _calculate_promotion_cashback(
        ...
        return self._calculate_promotion_cashback_amount(purchase_amount, cashback_config)
    ...
    def _calculate_promotion_cashback_amount(
        ...
        if (cashback_config.cashback_type == CashbackTypeEnum.PERCENTAGE and
            cashback_config.cashback_percentage is not None):
            cashback_amount = (
                purchase_amount * cashback_config.cashback_percentage
            ) / decimal.Decimal("100")

        elif (cashback_config.cashback_type == CashbackTypeEnum.FIXED_AMOUNT and
              cashback_config.cashback_amount is not None):
    ...
    async def _calculate_base_cashback(
        ...
        base_config = await self.dao.default_company_cashback_config.get_active_by_company_id(
    ...
    async def _create_transaction_records(
        ...
        created_transaction = await self.dao.transaction.create(session, obj_in=transaction_data)
