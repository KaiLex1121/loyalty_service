...
from backend.dao.holder import HolderDAO # Предполагаем, что HolderDAO принимает сессию при инициализации или DAO получают сессию в методах
...
class AdminTariffPlanService:
    async def _load_plan_for_response(self, session: AsyncSession, dao: HolderDAO, plan_id: int) -> Optional[TariffPlanModel]:
    ...
    async def create_tariff_plan(
        ...
        async with session.begin(): # <--- ТРАНЗАКЦИЯ ЗДЕСЬ (в сервисе)
            existing_plan = await dao.tariff_plan.get_by_name(session, name=plan_data.name)
            if existing_plan:
                raise HTTPException(
    ...
    async def get_all_tariff_plans(
        ...
        return await dao.tariff_plan.get_multi(session, skip=skip, limit=limit, include_deleted=True)

    async def update_tariff_plan(
        self, session: AsyncSession, dao: HolderDAO, plan_id: int, plan_update_data: TariffPlanUpdate
    ) -> Optional[TariffPlanModel]:
        async with session.begin(): # <--- ТРАНЗАКЦИЯ ЗДЕСЬ
            ...
            if plan_update_data.name and plan_update_data.name != plan_to_update.name:
                ...
                existing_plan_with_new_name = await dao.tariff_plan.get_by_name(session, name=plan_update_data.name)
                if existing_plan_with_new_name and existing_plan_with_new_name.id != plan_id:
                    raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Tariff plan with name '{plan_update_data.name}' already exists.")


            if plan_update_data.is_trial is not None and plan_update_data.is_trial and \
               (plan_update_data.status == TariffStatusEnum.ACTIVE if plan_update_data.status else plan_to_update.status == TariffStatusEnum.ACTIVE):
                # ... (проверка на другой активный триал) ...
                active_trial_plan = await dao.tariff_plan.get_trial_plan(session)
                if active_trial_plan and active_trial_plan.id != plan_id:
                    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="An active trial plan already exists.")

            # dao.tariff_plan.update НЕ должен делать commit
            updated_plan = await dao.tariff_plan.update(session, db_obj=plan_to_update, obj_in=plan_update_data)
        ...
        async with session.begin(): # <--- ТРАНЗАКЦИЯ ЗДЕСЬ
            ...
            if plan_to_archive.status != TariffStatusEnum.ARCHIVED:
                ...
                archived_plan = await dao.tariff_plan.update(
                    ...
                    obj_in=TariffPlanUpdate(status=TariffStatusEnum.ARCHIVED, is_public=False) # Передаем схему обновления
                )
