...
from app.bots.customer_bot.text.formatters.general import format_promotions, format_transaction_history
...
async def show_profile(callback: CallbackQuery, bot: Bot, state: FSMContext, company_id: int, api_client: CoreApiClient):
    profile = await api_client.get_customer_profile(callback.from_user.id, company_id)
    if profile:
            await callback.message.edit_text(f"Ваш профиль:", reply_markup=ViewProfileKeyboards.view_profile_keyboard)
    else:
...
async def show_promotions(callback: CallbackQuery, bot: Bot, state: FSMContext, company_id: int, api_client: CoreApiClient):
    ...
    await callback.message.answer("Это все действующие акции:", reply_markup=MainMenuKeyboards.back_to_main_menu_keyboard)
