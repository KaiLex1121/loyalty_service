...
from shared.schemas import TokenPayload # Предполагаем, что нужна будет модель сотрудника
...
async def get_current_employee(state: FSMContext, message: Message) -> Optional[Employee]:
    """
    ...
    if not payload:
        ...
        await state.clear()
        await message.answer("Ваша сессия истекла. Пожалуйста, войдите снова, поделившись номером.")
        return None
