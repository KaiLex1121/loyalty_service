...
from app.schemas.employee_bot_auth import EmployeeOtpRequest, EmployeeOtpVerify, EmployeeAuthResponse
...
from app.core.dependencies import get_employee_auth_service
...
async def request_employee_otp(
    ...
    otp = await service.request_otp_for_employee_login(session, request_data.work_phone_number, company_id)
    # ВАЖНО: В проде этот ответ должен быть пустым (204 No Content)
    return {"detail": "OTP has been sent.", "otp_for_testing": otp}


@router.post("/auth/verify-otp", response_model=EmployeeAuthResponse, summary="Verify OTP and get JWT")
