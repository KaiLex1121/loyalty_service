...
@router.post(
    "", response_model=TelegramBotResponse, status_code=status.HTTP_201_CREATED
)
async def create_bot(
    ...
    new_bot = await bot_service.create_bot(
        session, bot_data=bot_in, company_id=company.id
    )
    return new_bot
