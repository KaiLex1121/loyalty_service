...
class TelegramBotDAO(
    BaseDAO[TelegramBot, TelegramBotCreateInternal, TelegramBotUpdate]
):
    def __init__(self):
    ...
    async def find_soft_deleted_by_token(
        self, session: AsyncSession, token: str
    ) -> Optional[TelegramBot]:
        """Находит мягко удаленного бота по токену."""
        stmt = select(self.model).where(
            self.model.token == token, self.model.deleted_at.is_not(None)
        )
        result = await session.execute(stmt)
        return result.scalars().first()
    ...
    async def get_all_active_with_company(
